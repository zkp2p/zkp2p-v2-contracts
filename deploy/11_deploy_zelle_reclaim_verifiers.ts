...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  // Add verifiers to whitelist
  const escrowContract = await ethers.getContractAt("Escrow", escrowAddress);
  await addWhitelistedPaymentVerifier(hre, escrowContract, chaseVerifier.address, ZELLE_RECLAIM_FEE_SHARE[network]);
  await addWhitelistedPaymentVerifier(hre, escrowContract, citiVerifier.address, ZELLE_RECLAIM_FEE_SHARE[network]);
  await addWhitelistedPaymentVerifier(hre, escrowContract, boaVerifier.address, ZELLE_RECLAIM_FEE_SHARE[network]);
  console.log("Zelle verifiers added to whitelisted payment verifiers...");
...
func.skip = async (hre: HardhatRuntimeEnvironment): Promise<boolean> => {
  ...
  if (network != "localhost") {
    try { 
      getDeployedContractAddress(hre.network.name, "ZelleChaseReclaimVerifier");
      getDeployedContractAddress(hre.network.name, "ZelleCitiReclaimVerifier");
      getDeployedContractAddress(hre.network.name, "ZelleBoAReclaimVerifier");
      return true;
